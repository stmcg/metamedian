...
#'
#' This function is a wrapper function for the \code{\link{qe}} and \code{\link{pool.med}} functions. The function implements the methods of McGrath et al. (2019) and McGrath et al. (2020) to estimate the pooled (difference of) medians in a meta-analysis. Specifically, the function implements the (weighted) median of medians method and quantile matching estimation method to meta-analyze one-group studies; the function implements the (weighted) median of the difference of medians method and quantile matching estimation method to meta-analyze two-group studies.
#'
...
#' \code{qe} \tab Quantile Matching Estimation (McGrath et al. 20220). This is the default option. \cr}
...
#' @param norm.approx (only applicable when \code{median_method} is set to \code{mm}) logical scalar indicating whether normality approximation of the binomial should be used to construct an approximate confidence interval. The default is \code{TRUE}.
#' @param coverage.prob (only applicable when \code{median_method} is set to \code{mm}) numeric scalar indicating the desired coverage probability. The default is \code{0.95}.
...
#'
#' @return an object of class "rma.uni" (when \code{median_method} is set to \code{qe}) or a list (when \code{median_method} is set to \code{mm} or \code{wm}). For additional details, see the \code{\link[metafor]{rma.uni}} and \code{\link{pool.med}} functions, respectively.
...
metamedian <- function(df, median_method = 'qe', single.family = FALSE,
  ...
  if (!median_method %in% c('qe', 'mm', 'wm')){
    stop("median_method must be set to 'mm', 'wm', or 'qe'")
  }
